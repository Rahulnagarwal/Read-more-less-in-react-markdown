...
export default function Markdown({ children, ...other }) {
  const [showFullContent, setShowFullContent] = useState(!(children?.length > 300));
  ...
  return (
    <MarkdownStyle>
      ...
      <ReactMarkdown rehypePlugins={[rehypeRaw]} components={components} {...other} >
        {showFullContent ? children : `${children.slice(0, 300)}...`}
      </ReactMarkdown>
      {!showFullContent && (
        // eslint-disable-next-line react/button-has-type
        <button onClick={toggleShowFullContent}>Read More</button>
      )}
      {children?.length > 300 && showFullContent && (
        // eslint-disable-next-line react/button-has-type
        <button onClick={toggleShowFullContent}>Read less</button>
      )}
    </MarkdownStyle>
  );
}
